...
RF_QUAY_USERNAME=$(grep '\.dockerconfigjson' secret.yaml | awk '{print $2}' | sed 's/"//g' | base64 -d | jq -r '.auths["quay.io"].auth' | base64 -d | awk -F: '{print $1}')
RF_QUAY_PASSWORD=$(grep '\.dockerconfigjson' secret.yaml | awk '{print $2}' | sed 's/"//g' | base64 -d | jq -r '.auths["quay.io"].auth' | base64 -d | awk -F: '{print $2}')
